        ...
        if (effectFlag && socket) {
            socket.on("real-time", (data: { time: number, gameHistory: GameHistoryType[] }) => {
                ...
                if (data.time === 0) {
                    ...
                    setCurCard(data?.gameHistory?.[0]?.id >= 0 ? data?.gameHistory?.[0]?.id : -1)
                    setStopCard(false)
    ...
    return (
        <Stack className="content">
            <Stack className="card-history">
                {HistoryCard?.map(
                    (item: HistoryCardsType, key: number) => {
                        if (item.id === 0) {
                            return (
                                <Stack key={key} className="card-small">
                                    <img src="https://nar-fg.cchhllpp.net/prd/images/hilo/svg/joker.svg" alt="img" className="joker-card" />
                                    <img src={item.flag === 0 ? EqualImg : item.flag === 2 ? HiImg : LoImg} alt="img" />
                                </Stack>
                            );
                        } else {
                            return (
                                <Stack key={key} className="card-small">
                                    <Typography sx={{ color: item.id > 12 ? "red" : "black" }}>{item.title}</Typography>
                                    <img src={item.flag === 0 ? EqualImg : item.flag === 2 ? HiImg : LoImg} alt="img" />
                                </Stack>
                            );
                        }
            ...
            <Stack className="card-main">
                <Box className="card-animation-scene" ref={CardRef}>
                    <Box className="card-front">
                        <img src={`/Card/${curCard}.png`} alt="card" className="playing-card" />
                        <span className="loader" style={{ display: stopCard ? "none" : "block" }}></span>
                        <span className="time-count" style={{ display: stopCard ? "none" : "block" }}>{timeCount}</span>
                    </Box>
