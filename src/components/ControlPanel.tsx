...
const ControlPanel: React.FC<IControlPanel> = ({ hPercent, lPercent, hMulti, lMulti, betAmount, setBetAmount }: any) => {
    // @ts-ignore
            ...
            if (selectedId !== undefined && selectedId !== 100) {
                if (betAmount < 50) {
                    ...
                    let gameResult = await axios.post(`${process.env.REACT_APP_SERVER_URL}/api/game/bet-game-ready`, {
                        userId: 71028664, selectedId, betAmount, uuid: sessionStorage.getItem("hilo-uuid")
                    })
    ...
    return (
        <Stack spacing={2}>
            <Stack className="control-box" sx={{ pointerEvents: disableBet ? "none" : "all" }}>
                <Stack
                    direction="row"
                    alignItems="center"
                ...
                <Grid container>
                    <Grid md={4} item flex={1}>
                        <Stack className="control-tab">
                            {ColorCategories.map(
                                (item: ColorCategoriesType, key: number) => {
                                    return (
                                        <Stack
                                            flex={1}
                                            key={key}
                    ...
                    <Grid md={4} item flex={1}>
                        <Stack className="control-tab">
                            {HiLoCategories.map(
                                (item: HiLoCategoriesType, key: number) => {
                                    return (
                                        <Stack
                                            key={key}
                                            flex={1}
            ...
            <Stack className="control-box input-box" sx={{ pointerEvents: disableBet || isBetted ? "none" : "all" }}>
                ...
                <Stack pt={1} spacing={1}>
                    <Button variant="contained" className="btn-bet" sx={{ pointerEvents: disableBet ? "none" : "all" }} onClick={StartBet}>
