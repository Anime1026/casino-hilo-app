...
import { Categories, ColorCategories, HiLoCategories, OptionCategories } from '../config';
...
const columns: readonly Column[] = [
    ...
    { id: 'betAmount', label: 'Amount', minWidth: 100, format: (value: number) => value.toFixed(), },
    {
        id: 'isWin',
        label: 'Won',
        minWidth: 100,
...
const columnsHistory: readonly Column[] = [
    ...
    { id: 'betAmount', label: 'Bet Amount', minWidth: 100, format: (value: number) => value.toFixed(), },
    {
        id: 'isWin',
        label: 'Won',
        minWidth: 100,
    ...
    return (
        <>
            <AppBar position='static' color="primary" className='game-history'>
                <Stack spacing={2} direction="row" alignItems="left" className="history-appbar">
                    <Button variant="contained" className={active ? 'current-bet-active' : 'current-history-Unactive'} onClick={() => {
                        setActive(true)
                    }}>Current Bet</Button>
                    <Button variant="contained" className={!active ? 'current-bet-active' : 'current-history-Unactive'} onClick={() => {
                        setActive(false)
                                            ...
                                            return (
                                                <TableRow hover role="checkbox" tabIndex={-1} key={key}>
                                                    {columns.map((column: any) => {
                                                        ...
                                                        if (column.id === 'isWin') {
                                                            return (
                                                                <TableCell key={column.id} align={column.align} sx={{ color: value ? 'green' : 'red' }}>
                                                                    {value === -1 ? 'Ready!' : value}
                                                                </TableCell>
                                                            )
                                                        } else if (column.id === "betId") {
                                                            const categoriesArray = [Categories, ColorCategories, OptionCategories, HiLoCategories];

                                                            for (let i = 0; i < categoriesArray.length; i++) {
                                                                const foundItem = categoriesArray[i].find(item => item.id === value);
